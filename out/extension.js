...
class SkinkDebugConfigurationProvider {
    ...
    async resolveDebugConfigurationWithSubstitutedVariables(folder, config, token) {
        ...
        vscode.debug.startDebugging(folder, nativeConfig, { noDebug: config.noDebug === true }).then((started) => {
            if (!started) {
                output.appendLine(`[Skink Build Error] Failed to start the "${nativeConfig.type}" debug session.`);
                vscode.window.showErrorMessage(`Failed to start the native debugger ("${nativeConfig.type}"). See the Skink Build output for details.`);
            }
        }, (err) => {
